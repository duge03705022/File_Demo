...
public class CardHandler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        selectMap = new bool[RFIBParameter.stageCol, RFIBParameter.stageRow];
    ...
    private void updateCards()
    {
        for (int i = 0; i < RFIBParameter.stageCol; i++)
        {
            for (int j = 0; j < RFIBParameter.stageRow; j++)
            {
                ...
                if (rFIBManager.blockId[i, j, 0] != "0000")
                {
                    if (rFIBManager.blockId[i, j, 1] == "7601")
                    {
                        if (gameController.selectCount == 0 || (i == gameController.sourceX && j == gameController.sourceY))
                        {
                            cardInstance[i, j].GetComponent<FileController>().OpenSelect();
                        }
                        else
                        {
                            cardInstance[i, j].GetComponent<FileController>().AddTxt();
                            cardInstance[i, j].GetComponent<FileController>().ifOpen = true;
                            cardInstance[i, j].GetComponent<FileController>().txtParent.SetActive(true);
                            cardInstance[i, j].GetComponent<FileController>().ShowPage();
                        }
                        selectMap[i, j] = true;
                    }
                    else
                    {
                        if (selectMap[i, j])
                        {
                            cardInstance[i, j].GetComponent<FileController>().CloseSelect();
                            selectMap[i, j] = false;
                        }
                    }
                }
